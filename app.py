...
# ================= Simpanan & Laporan =================
    ...
    if gaji_data.empty or belanja_data.empty:
        ...
        with st.expander("🔍 Tapis Laporan"):
            ...
            with col1:
                tahun_list = sorted(set(gaji_data['Tahun'].unique()) | set(belanja_data['Tahun'].unique()))
                tahun_selected = st.selectbox("Tahun", tahun_list)
            
            with col2:
                nama_list = ["Semua"] + sorted(gaji_data['Nama'].unique())
                nama_selected = st.selectbox("Nama", nama_list)
            
            with col3:
                bulan_list = ["Semua"] + ["Jan", "Feb", "Mac", "Apr", "Mei", "Jun", 
                                        "Jul", "Ogos", "Sep", "Okt", "Nov", "Dis"]
                bulan_selected = st.selectbox("Bulan", bulan_list)
            
            target_simpanan = st.number_input("🎯 Sasaran Simpanan Bulanan (RM)", 
        ...
        laporan = pd.DataFrame({
            'Tahun': filtered_gaji['Tahun'],
            'Bulan': filtered_gaji['Bulan'],
            'Jumlah Gaji': filtered_gaji['Gaji Bersih'],
            'Jumlah Belanja': filtered_belanja.groupby(['Tahun', 'Bulan'])['Jumlah'].sum().values,
            'Sasaran Simpanan': target_simpanan
        }).fillna(0)
        
        laporan['Simpanan'] = laporan['Jumlah Gaji'] - laporan['Jumlah Belanja']
        laporan['Beza Sasaran'] = laporan['Simpanan'] - laporan['Sasaran Simpanan']

        st.subheader("📋 Ringkasan Kewangan")
        st.dataframe(laporan.style.format("{:,.2f}"), use_container_width=True)

        # Charts
        col1, col2 = st.columns(2)
        with col1:
            st.subheader("💰 Trend Simpanan")
            st.line_chart(laporan.set_index('Bulan')[['Simpanan', 'Sasaran Simpanan']])
        
        with col2:
            st.subheader("📊 Agihan Belanja")
        ...
        total_gaji = laporan['Jumlah Gaji'].sum()
        total_belanja = laporan['Jumlah Belanja'].sum()
        ...
        col3.metric("Jumlah Simpanan", f"RM {total_simpanan:,.2f}", 
                   delta=f"{total_simpanan/target_simpanan*100:.1f}% dari sasaran")
